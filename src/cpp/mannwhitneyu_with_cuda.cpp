...
PARALLEL_REGION(threads)
    ...
    const auto [min, max] = simd::array_reduce_minmax(begin, len);
    mins[tid] = min;
    maxs[tid] = max;
PARALLEL_END(old_threads);
    const auto max = simd::array_reduce_max<uint32_t>(maxs, threads);
    if unlikely_(max < max_clip) return {max, max}; // dont need to calculate
    const auto min = simd::array_reduce_min<uint32_t>(mins, threads);
    ...
    // 3. hist
    const auto global_min_v = hn::Set<float>((float)min);
